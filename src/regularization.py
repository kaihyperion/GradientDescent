...
class L1Regularization(Regularization):
    ...
    def forward(self, w):
        ...
        w_abs = np.abs(w[:-1])
        regularization_term = np.sum(w_abs) * self.reg_param
    ...
    def backward(self, w):
        ...
        Returns:
            gradient_term - (np.array) A numpy array of length d+1. The
                gradient of the regularization term evaluated at w.
        """
        gradient_term = np.zeros(w.shape[0])
        for p in range(w.shape[0]-1):
            if w[p] == 0:
                continue
            elif w[p] > 0:
                gradient_term[p] = 1 * self.reg_param
            else:
                gradient_term[p] = -1 * self.reg_param
...
class L2Regularization(Regularization):
    ...
    def forward(self, w):
        ...
        Returns:
            regularization_term - (float) The value of the regularization term
                evaluated at w.
        """
        w_abs_sq = np.square(np.abs(w[:-1]))
        regularization_term = (np.sum(w_abs_sq) * 0.5) * self.reg_param
        return regularization_term
    ...
    def backward(self, w):
        ...
        gradient_term = np.zeros(w.shape[0])
        for p in range(w.shape[0] - 1):
            gradient_term[p] = w[p] * self.reg_param
