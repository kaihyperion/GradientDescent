...
class MultiClassGradientDescent:
    ...
    def fit(self, features, targets, batch_size=None, max_iter=1000):
        ...
        for tar_ind in range(self.classes.shape[0]):
            belong = np.ones(targets.shape[0])
            for i in range(targets.shape[0]):
                if targets[i] != self.classes[tar_ind]: belong[i] = -1
            if self.regularization:
                gd = GradientDescent(loss=self.loss, regularization=self.regularization, reg_param=self.reg_param)
            else: gd = GradientDescent(self.loss)
            gd.fit(features, belong, batch_size, max_iter)
    ...
    def predict(self, features):
        ...
        possibilities = np.zeros((self.classes.shape[0], features.shape[0]))
        for c in range(self.classes.shape[0]):
            out = self.model[c].confidence(features)
            possibilities[c] = out
        predictions_ind = np.argmax(possibilities, axis= 0)
        for p in range(predictions_ind.shape[0]):
            predictions[p] = self.classes[predictions_ind[p]]
        return predictions
